...
$(function () {
	...
	$.ajax({
		type: "get",
		url: "/ssm_pro/doctorOrderShow/" + doctorId,
		ContentType: "text/html;charset=UTF-8",
		success: function (msg) {
			console.log(msg)
			intoTempArr = msg;
			for (let i = 1; i < 31; i++) {
				if (intoTempArr.length == 0) {
					dp.PArr.push(new specialDate(addDate(myDate, i), {
						's': true,
						'z': true,
						'x': true
					}));
					...
					let flg = false
					let index = 0;
					for (let m = 0; m < intoTempArr.length; m++) {
						if (addDate(myDate, i) == intoTempArr[m].transactDate) {
							index = m;
							flg = true
							break;
						}
					...
					if (flg) {
						dp.PArr.push(new specialDate(addDate(myDate, i), {
							's': intoTempArr[index].s,
							'z': intoTempArr[index].z,
							'x': intoTempArr[index].x
						}));
					} else {
						dp.PArr.push(new specialDate(addDate(myDate, i), {
							's': true,
